        ...
        <div class="card-body">
            <div class="container-fluid">
                <div class="card">
                    <div class="card-body">
                        <div class="col-lg-6">
                            <form wire:submit.prevent="{{ $phaseId ? 'updatePhase' : 'createPhase' }}">
                                <div class="col-lg-6">
                                    <label for="">{{ __('Phase Code') }}</label>
                                    <input type="text" wire:model="code" placeholder="{{ __('Phase Code') }}"
                                        class="form-control">
                                    @error('code')
                                    ...
                                    <label for="">{{ __('Phase Name') }}</label>
                                    <input type="text" wire:model="name" placeholder="{{ __('Phase Name') }}"
                                        class="form-control">
                                    @error('name')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                ...
                                <button type="submit" class="btn btn-primary btn-rounded mt-2">
                                    <i class="fa fa-save"> </i> {{ $phaseId ? __('Update') : __('Save') }}
                                </button>
                            </form>
                        </div>
                    </div>
            ...
            <ul class="list-group">
                @foreach ($phases as $phase_val)
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <div class="col-10">
                            <strong class="text-dark">{{ $phase_val->name }}</strong><br>
                            <small class="text-dark">{{ $phase_val->code }}</small>

                        </div>
                        <div class="col-2 d-flex justify-content-end">
                            <button wire:click="editPhase({{ $phase_val->id }})"
                                class="btn btn-sm btn-dark text-white btn-rounded">{{ __('Update') }}</button>&nbsp;
                            <button wire:click="deletePhase({{ $phase_val->id }})"
