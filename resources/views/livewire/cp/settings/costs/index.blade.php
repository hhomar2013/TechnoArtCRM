<div>
    <div class="card">
        <div class="card-header">
            <h4> <i class="fa-solid fa-bars"></i> {{ __('Costs') }}</h4>
        ...
        <div class="card-body">
            <div class="container-fluid">
                <div class="card">
                    <div class="card-body">
                        <div class="col-lg-6">
                            <form wire:submit.prevent="{{ $costId ? 'updateCosts' : 'createCosts' }}">
                                <div class="row">
                                    <div class="col-lg-6">
                                ...
                                <button type="submit" class="btn btn-primary btn-rounded mt-2">
                                    <i class="fa fa-save"> </i> {{ $costId ? __('Update') : __('Save') }}
                                </button>
                            </form>
                        </div>
                    </div>
            ...
            <ul class="list-group">
                @foreach ($costs as $cost)
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <div class="col-10">
                            <strong class="text-dark">{{ $cost->name }}</strong><br>
                            {{-- <small class="text-dark">{{ $bank->code }}</small> --}}

                        </div>
                        <div class="col-2 d-flex justify-content-end">
                            <button wire:click="editCosts({{ $cost->id }})"
                                class="btn btn-sm btn-dark text-white btn-rounded">{{ __('Update') }}</button>&nbsp;
                            <button wire:click="deleteCosts({{ $cost->id }})"
