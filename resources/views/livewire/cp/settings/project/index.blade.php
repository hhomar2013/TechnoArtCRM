<div>
    <div class="card">
        <div class="card-header">
            <h4><i class="fa-solid fa-bars"></i>{{ __('Projects Management') }}</h4>
        ...
        <div class="card-body">
            <div class="container-fluid">
                <div class="card">
                    <div class="card-body">
                        <div class="col-lg-6">
                            <form wire:submit.prevent="{{ $projectId ? 'updateProject' : 'createProject' }}">
                                <div class="col-lg-6">
                                    <label for="">{{ __('Project Code') }}</label>
                                    <input type="text" wire:model="code" placeholder="{{ __('Project Code') }}"
                                        class="form-control">
                                    @error('code')
                                    ...
                                    <input type="text" wire:model="name" placeholder="{{ __('Project Name') }}"
                                        class="form-control">
                                    @error('name')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                ...
                                <button type="submit" class="btn btn-primary btn-rounded mt-2">
                                    <i class="fa fa-save"> </i> {{ $projectId ? __('Update') : __('Save') }}
                                </button>
                            </form>
                        </div>
                    </div>
            ...
            <ul class="list-group">
                @foreach ($projects as $project)
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <div class="col-10">
                            <strong class="text-dark">{{ $project->name }}</strong><br>
                            <small class="text-dark">{{ $project->code }}</small>
