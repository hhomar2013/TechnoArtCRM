<div>
    ...
    <div class="card text-dark">
        ...
        <div class="card-header">
            {{ __('Costs') }}
            <button class="btn btn-warning btn-rounded" wire:click="back">{{ __('Back') }} <i class="fa fa-arrow-left"></i></button>
        </div>
        @else
        <div class="card-header">
            {{ __('Payments') }}
        ...
        <div class="card-body">
            <div class="basic-list-group">
                <ul class="list-group">
                    ...
                    <?php $value += $val['value']; ?>
                    <li wire:key="cost-{{ $index }}" class="list-group-item d-flex justify-content-between align-items-center">
                        {{ $val['cost'] }} <br>
                        {{ $val['date'] }} <br>
                        {{ number_format($val['value'], 2) }} &nbsp;
                        <button class="btn btn-danger btn-sm" wire:click="removeCost({{ $index }})">-</button>
                    </li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="basic-list-group">
                <ul class="list-group">
                    ...
                    <li class="list-group-item d-flex justify-content-between align-items-center" wire:key="payment-{{ $payment_index }}">
                        {{ $payment_val['type'] }} <br>
                        {{ $payment_val['date'] }} <br>
                        {{ number_format($payment_val['amount'], 2) }} &nbsp;
                        <button class="btn btn-danger btn-sm" wire:click="removePayment({{ $payment_index }})">-</button>
                    </li>
                    @endforeach
                                ...
                                <form wire:submit.prevent="{{ $collectCosts ? 'saveCosts' : 'savePayments' }}">
                                    ...
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="number" class="form-control" placeholder="{{ __('Receipt number') }}" wire:model="transaction_id">
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="date" class="form-control" placeholder="{{ __('Receipt date') }}" wire:model="transaction_date">
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="number" class="form-control" placeholder="{{ __('Total receipt') }}" wire:model.live="receipt_value" min="1">
                                            @error('receipt_value')
                                            <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                            <br>
                    ...
                    <div id="rounded-stylish_collapseOne" class="accordion__body collapse show" data-parent="#accordion-eleven" style="">
                        <div class="accordion__body--text">
                            <div class="row">
                                ...
                                <select name="" id="" class="form-control" wire:model.live="customer">
                ...
                <div class="basic-list-group">
                    <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <button class="btn btn-danger btn-rounded text-white" wire:click="previewCustomerReport({{ $greaterThanCustomerCount_val->installment_plan_id }})">
                        ...
                        <table class="table table-hover text-dark">
                            ...
                            <tbody>
                                ...
                                <td>
                                    ...
                                    <span class="badge badge-danger text-white">{{ __('Partially collected') }}
                                    </span>
                                    {{-- <br>
                                                    <span>
                                ...
                                <td>
                                    <button type="submit" class="btn btn-dark btn-rounded text-white " {{ $selectedPayments ? 'hidden' : '' }} {{ $cost_val->status == 'paid' ? 'hidden' : '' }} @if ($cost_val->status == 'partiallycollected') wire:click="selectCostReaming({{ $cost_val->id }})">
                                        @else
                    ...
                    <div class="table-responsive " style="overflow-y: auto; height: 400px;">
                        <table class="table table-hover text-dark">
                            ...
                            <tbody>
                                ...
                                <td> <span class="badge badge-primary">{{ $payment_val->type == 'installment' ? 'قسط' : '' }}</span>
                                </td>
                                ...
                                <td>
                                    ...
                                    <span class="badge badge-danger text-white">{{ __('Partially collected') }}</span>
                                    {{-- <span>
                                                        <b class="text-white badge badge-danger">
                                                            {{ __('reaming') }}
                                ...
                                <td>
                                    ...
                                    <button type="submit" class="btn btn-dark btn-rounded text-white " {{ $selectedCosts ? 'hidden' : '' }} {{ $payment_val->status == 'paid' ? 'hidden' : '' }} @if ($payment_val->status == 'partiallycollected') wire:click="selectPaymentReaming({{ $payment_val->id }})">
                                        @else
        ...
        <div class="card-footer">
            <div class="row">
                <div class="col-lg-6">
                    ...
                    <hr>
                    <button class="btn btn-primary" wire:click.prevent="$set('collectCosts',true)">{{ __('Collect') . ' ' . __('Costs') }}<i class="fa-solid fa-money-bill"></i></button>
                ...
                <div class="col-lg-6">
                    ...
                    @if (!$selectedCosts && $selectedPayments)
                    <hr>
                    <button class="btn btn-primary" wire:click.prevent="collectPayment">{{ __('Collect') . ' ' . __('Installments') }}<i class="fa-solid fa-money-bill"></i></button>
