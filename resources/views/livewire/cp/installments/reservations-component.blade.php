                    ...
                    <div class="card-body">
                        ...
                        <div class="default-tab">
                            <ul class="nav nav-tabs" role="tablist">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link {{ $tabs == 'costs_installments' ? 'active' : 'disabled' }}" data-toggle="tab" href="#costs_installments" wire:click="changeTabs('costs_installments')">{{ __('Costs') . ' & ' . __('Installments') }}</a>
                                </li>
                            </ul>
                            <div class="tab-content">
                                <div class="tab-pane fade {{ $tabs == 'home' ? 'active show' : '' }}" id="home" role="tabpanel">
                                    @foreach ($results as $plan)
                                    @if ($plan->status == 'transferred')
                                    <div class="card mb-3  bg-warning text-white }}" wire:key="card-{{ $plan->id }}">
                                        <div class="card-header">
                                            <h1 class="text-danger">{{ __('Transfer') }}</h1>
                                        </div>
                                        <div class="card-body">
                                            <div class="row">
                                                <div class="col-lg-6">
                                ...
                                <div class="tab-pane fade {{ $tabs == 'info' ? 'active show' : '' }}" id="info">
                                    <div class="pt-4 text-dark">
                                        ...
                                        <button class="btn btn-rounded btn-dark text-white" wire:click="changeTabs('costs_installments')">
                                            <i class="fa fa-users"></i>
                                            {{ __('Show Costs & Installments') }} </button>

                                        <button class="btn btn-danger btn-rounded text-white" wire:click="Transfare({{ $reservationId }})">
                                            <i class="fa-solid fa-arrow-right-arrow-left"></i>
                                            {{ __('Transfer') }}</button>
                                        <hr>
                                        @if ($transfare)
                                        @include('livewire.cp.installments.transfare')
                                        @else
                                        @if ($newCustomer)
                                        <div class="card">
                                            <div class="card-header">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-6">
                                                ...
                                                <select wire:model.live="selectedCustomerType" class="form-control" name="" id="">
                                                ...
                                                @enderror
                                                <br>
                                                <button class="btn btn-danger btn-sm btn-rounded" wire:click="saveWithdrawal">
                                            ...
                                            <div class="col-lg-6">
                                                <p># {{ $i++ }} </p>
                                                <p>{{ __('Customer Code') }} : {{ $info->customer->code }}
                                                </p>
                                                <p>{{ __('Customer Name') }} : {{ $info->customer->name }}
                                                </p>
                                                <p>{{ __('Requested Area') }} : {{ $info->customer->area }} م</p>
                                            </div>
                                            <div class="col-lg-6">
                                                <button class="btn btn-danger btn-sm btn-rounded" wire:click="WithDrawal({{ $info->customer->code }})">
                                                    <i class="fa-solid fa-user-slash"></i>
                                ...
                                <div class="tab-pane fade {{ $tabs == 'costs_installments' ? 'active show' : '' }}" id="costs_installments">
